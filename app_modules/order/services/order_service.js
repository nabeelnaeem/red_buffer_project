    ...
    try {
        ...
        for (const item of cart) {
            ...
            await sequelize.query(`
                ...
                `, {
                replacements: {
                    order_id, product_id: item.product_id, quantity: item.quantity, amount
                },
                transaction: t
            });
            await sequelize.query(`
        ...
        //Shipping
        const tracking_id = `TRK-${uuidv4().slice(0, 8).toUpperCase()}`// Human readable 8 characters tracking id
        ...
        //Updating user info if missing
        const [userResult] = await sequelize.query(`
            SELECT full_name, address, phone FROM users
